...
with tab1:
    ...
    with st.expander("➕ 新しい予定を追加"):
        input_date = st.date_input("日付", datetime.now(), key="new_ev_date")
        input_ev = st.text_input("予定内容", key="new_ev_text")
        if st.button("予定を保存"):
...
with tab3:
    ...
    if not df_tasks.empty:
        ...
        if not incomplete.empty:
            ...
            st.success("未完了の課題はありません！")
    
    with st.expander("➕ 新しい課題を追加"):
        with st.form("task_form"):
            # 教科のリストをPC版（時間割）に合わせて拡充
            t_sub = st.selectbox("教科", [
                "科技α", "科技β", "数基α", "数基β", 
                "言語文化", "現国", "論表", "コミュ I", 
                "歴総", "地総", "保健", "体育", "家庭", "音楽", "探求基礎", "その他"
            ])
